...
import com.example.log.model.SpuInfo
...
object SpuInfoApp {
  ...
  def main(args:Array[String]):Unit = {
    ...
    val topic = "ods_maxwell_spu_info"
    val groupId = "spu_info_app_group"
    val phoenixTable = "gmall_spu_info"
    ...
    // 转换为对象流
    val spuInfoStream = extractedKafkaStream.map{record =>
      JSON.parseObject(record.value(),classOf[SpuInfo])
    }

    // 写入phoeinx,并在每个采集周期保存offset

    spuInfoStream.foreachRDD{rdd =>
      rdd.saveToPhoenix(
        phoenixTable,
        Seq("ID","SPU_NAME"),
        new Configuration(),
