...
import com.example.log.util.{MyESUtil, MyKafkaUtil, MyMysqlUtil, MyRedisUtil, OffsetManagerUtil}
...
object C6TxConsumeOffset {

  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("dau-app").setMaster("local[*]")
    val ssc = new StreamingContext(conf, Seconds(5))
    val topic = "rt_start_log"
    val groupId = "dau-group"
    ...
    filtedStream.mapPartitions { iter =>
      val conn = MyMysqlUtil.getConnect()
      val ps = conn.prepareStatement("insert into gmall_2021_dau_info(mid,uid,ar,ch,vc,dt,hr,mi,ts) value(?,?,?,?,?,?,?,?,?)")
      ...
      try {
          iter.foreach{ obj =>
            ...
            ps.setString(1, dauInfo.mid)
            ps.setString(2, dauInfo.uid)
            ps.setString(3, dauInfo.ar)
            ps.setString(4, dauInfo.ch)
            ps.setString(5, dauInfo.vc)
            ps.setString(6, dauInfo.dt)
            ps.setString(7, dauInfo.hr)
            ps.setString(8, dauInfo.mi)
            ps.setLong(9, dauInfo.ts)
            ps.addBatch()
          }
        OffsetManagerUtil.saveOffsetToRedis(topic, groupId, offsetRanges)
        ps.executeBatch()
        conn.commit()
        Array.empty.iterator
      }catch {
        case e:Exception =>
          e.printStackTrace()
          conn.rollback()
