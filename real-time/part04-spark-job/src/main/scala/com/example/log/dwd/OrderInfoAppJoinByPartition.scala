 ...
 *
 */
object OrderInfoAppJoinByPartition {
  ...
  def main(args:Array[String]):Unit={
    ...
    val orderInfoStream = kafkaDStream.map{record =>
        val orderInfo = JSON.parseObject(JSON.parseObject(record.value()).toJSONString,classOf[OrderInfo])
    ...
    val orderWithProvinceStream = orderInfoStream.transform{rdd =>
      val province_ids = s"'${rdd.collect().map(_.province_id).mkString("','")}'"
      val sql = s"select ID,NAME,AREA_CODE,ISO_CODE from gmall_province_info where id in (${province_ids})"
      val provinceMap = MyPhoenixUtil.queryList(sql).map{row =>
        val id = row.getString("ID")
        val name = row.getString("NAME")
      ...
      val orderWithProvInfoRDD = rdd.map{order=>
        // 获取广播变量
    ...
    orderInfoWithFirstFlagDStream.filter(_.if_first_order=="1").print()
    orderInfoWithFirstFlagDStream.filter(_.if_first_order=="0").print()

    // step6: 将首单写入hbase
    fixFirstFlagOrderStream.foreachRDD{rdd =>
      rdd.filter(_.if_first_order=="1")
         .map{order =>
